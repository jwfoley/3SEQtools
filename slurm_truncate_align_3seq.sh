...
do 
	...
	echo "#! /bin/bash
		...
		cd $tmp_dir
		$unzip_path $fastq |
			$homopolymer_trim_path -p $N_A -m $N_mismatch -L $length 2> $wd/$rootname\_homopolymer_trim.log |
...
" | sbatch --cpus-per-task=$N_thread --job-name=$rootname\_align --output=$rootname\_job.log --time=$time --mail-type=$mail_type
done
