...
parse.align.log <- function(library.name) {
	log.list <- strsplit(sub(" \\(.+?\\)$", "", sub("^# *", "", grep(": ", readLines(get.filename(library.name, "align")), value = T))), ": *")
	log.vector <- sapply(log.list, function(x) x[2])
	names(log.vector) <- sapply(log.list, function(x) x[1])
	c(
		"input" =             as.integer(log.vector["Read Sequences"]),
		"uniquely aligned" =  as.integer(log.vector["Unique Alignment"]),
		"multiply aligned" =  as.integer(log.vector["Multi Mapped"]),
		"too short" =         as.integer(log.vector["Read Length"]),
		"other" =             as.integer(log.vector["No Mapping Found"]) + as.integer(log.vector["Homopolymer Filter"])
	)
}
